...
from app.models import SteelWip, QrCodes
...
async def get_filtered_wips(
    ...
    # 1. 기본 쿼리 생성: SteelWip과 QrCodes를 Outer Join (QR이 없는 재공품도 나올 수 있도록)
    stmt = select(SteelWip, QrCodes.qr_code).outerjoin(
        QrCodes, SteelWip.qr_id == QrCodes.id
    )
    ...
    wip_list = []
    for wip_obj, qr_val in result.all():
